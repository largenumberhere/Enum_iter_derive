...
fn impl_print_enum_variants_derive(ast: DeriveInput) -> proc_macro2::TokenStream{
    ...
    let enum_object = match ast.data {
        syn::Data::Enum(e) => e,
        other_type @ _ => panic!("Type '{:?}' is not supported by this macro", other_type.type_id())
    };

    let enum_variant_identifiers: Vec<Ident> = enum_object.variants
        .iter()
        .map(|v| v.ident.clone())
        .collect();

    let enum_variants_names: Vec<String> = enum_variant_identifiers.into_iter()
        .map(|i| i.to_string())
        .collect();
...
fn impl_flat_struct_values_derive(ast: DeriveInput) -> Result<proc_macro2::TokenStream, MacroError>{
    // Get struct
    ...
    // Get struct types
    let field_types: Vec<syn::Type> = struct_data.fields
        .iter()
        .map(|f| f.ty.clone())
        .collect();
...
fn impl_struct_to_iter(ast: DeriveInput) -> Result<proc_macro2::TokenStream, MacroError>{
    ...
    let enum_variant_names = {
        ...
        let idents: Vec<Ident> = idents.iter().map(|i| {
            let ident: Ident = syn::parse_str(i )
                .expect("Failed to parse enum_vairant_identifier token");
            ident
        }).collect() ;
